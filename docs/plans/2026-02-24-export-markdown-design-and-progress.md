...
### 5.1 TOC 跳转（2026-02-28）

- 当前实现仅支持 assistant 响应中的标准语义标题标签：`h1` ~ `h6`。
- 非标准“视觉标题”场景（例如 `div/span` + 样式模拟标题）不会进入 TOC，也无法跳转。
- 问题原因：跨平台 DOM 结构差异大，非语义标题缺少统一、稳定的解析锚点。
- TODO：按站点补充“非标准标题解析器”，并为每种站点定义可维护的 heading selector 策略与回退规则。
